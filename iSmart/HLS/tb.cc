...
uint256 DDR_dw1_pool_out_PL_burst[64/32*82*2*162*2];
uint256 DDR_dw2_pool_out_PL_burst[96/32*42*2*82*2];
uint256 DDR_buf_burst[128*44*84];
uint256 DDR_buf_merge[1048576];
...
int     image_raw_g_burst_int[3*162*2*324*2*2];
...
void load_weights()
{
    std::ifstream ifs_param("SkyNet_floating.bin", std::ios::in | std::ios::binary);
...
//    stitch_outputs(1, 1);
    ...
    for(int i = 0; i < 3*162*2*324*2*2; i++) {
    	if( i < 3*162*2*322*2 )
    		image_raw_g_burst_int[i] = image_raw_g_burst[i].to_int();
    	else
    		image_raw_g_burst_int[i] = 0;

    }


    SkyNet(image_raw_g_burst_int,
